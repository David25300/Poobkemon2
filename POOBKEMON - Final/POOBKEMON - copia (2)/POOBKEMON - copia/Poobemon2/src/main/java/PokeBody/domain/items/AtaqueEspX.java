// Archivo: PokeBody/domain/items/AtaqueEspX.java
...
public class AtaqueEspX implements Item {

    @Override
    public String getName() {
        return "X Sp. Atk"; // Nombre interno/en inglés
    }

    @Override
    public String getNombre() {
        return "Ataque Esp. X"; // Nombre en español para la UI
    }
    ...
    public void apply(Trainer user, Pokemon target, Consumer<String> messageConsumer) {
        ...
        int currentBoostLevel = target.getStatBoostLevel(StatBoosts.Stat.SP_ATTACK);
        if (currentBoostLevel >= 6) {
            messageConsumer.accept("¡El Ataque Especial de " + target.getNombre() + " no puede subir más!");
            return;
        }

        target.modificarStatBoost(StatBoosts.Stat.SP_ATTACK, 1);
        messageConsumer.accept(user.getName() + " usó " + getNombre() + " en " + target.getNombre() + ".");
        messageConsumer.accept("¡El Ataque Especial de " + target.getNombre() + " subió!");
