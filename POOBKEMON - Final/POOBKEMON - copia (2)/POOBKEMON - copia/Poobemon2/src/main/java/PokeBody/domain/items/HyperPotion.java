...
public class HyperPotion implements Item {
    ...
    public void apply(Trainer user, Pokemon target, Consumer<String> messageConsumer) {
        if (target == null) {
            messageConsumer.accept("No hay un Pokémon objetivo para usar la Hyperpocion.");
            return;
        }
        if (target.estaDebilitado()) {
            messageConsumer.accept("No se puede usar Hyperpocion: " + target.getNombre() + " está debilitado.");
            return;
        }
        ...
        target.curar(heal); 
        messageConsumer.accept(user.getName() + " usó Hyperpocion y restauró " + heal + " PS a " + target.getNombre() + "!");
